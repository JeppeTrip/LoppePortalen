...
export interface IStallTypeClient {

    createStallType(dto: CreateStallTypeRequest): Promise<CreateStallTypeResponse>;
}

export class StallTypeClient extends ClientBase implements IStallTypeClient {
    private http: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> };
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    createStallType(dto: CreateStallTypeRequest): Promise<CreateStallTypeResponse> {
        let url_ = this.baseUrl + "/api/StallType/create";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: Response) => this.processCreateStallType(_response));
        });
    }

    protected processCreateStallType(response: Response): Promise<CreateStallTypeResponse> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as CreateStallTypeResponse;
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<CreateStallTypeResponse>(null as any);
    }
}
...
export interface CreateStallTypeRequest {
    marketId?: number;
    name?: string | null;
    description?: string | null;
}
